...
contract TaxContract {
    ...
    function setRewardWallet(address rewardWallet_) public onlyOwner {
    ...
    TaxContract
{
    ...
    function _withdrawDividendOfUser(
        ...
        if (_withdrawableDividend > 0) {
            ...
            uint256 taxAmount = (_withdrawableDividend * claimTax) / 100;

            (bool userSuccess, ) = user.call{
                value: _withdrawableDividend - taxAmount
            }("");
            (bool taxSuccess, ) = payable(rewardsWallet).call{value: taxAmount}(
                ""
            );
...
contract RevShareStaking is ReentrancyGuard, DividendTracker {
    ...
    function deposit(uint256 _amount) external nonReentrant {
        ...
        uint256 taxAmount = (_amount * stakeTax) / 100;
        ...
        setBalance(
            payable(msg.sender),
            userAmount + (amountTransferred - taxAmount)
        );
    ...
    function compound(uint256 minOutput) external nonReentrant {
        ...
        if (amountEthForCompound > 0) {
            uint256 taxAmount = (amountEthForCompound * compoundTax) / 100;
            (bool taxSuccess, ) = payable(rewardsWallet).call{value: taxAmount}(
                ""
            );
            uint256 initialBalance = token.balanceOf(address(this));
            buyBackTokens(amountEthForCompound - taxAmount, minOutput);
            uint256 amountTransferred = token.balanceOf(address(this)) -
                initialBalance;
